      ...
      <View style={[styles.leftCloseButton, hovered && styles.hovered]} dataSet={{ media: ids.leftCloseButton }}>
        <Pressable onHoverIn={() => setHovered(true)} onHoverOut={() => setHovered(false)}>
          <TouchableHighlight onPress={() => console.log('hi')}>
            <MaterialCommunityIcons size={24} accessibilityHint='close' name='close' color={color.onSurface} />
          </TouchableHighlight>
        </Pressable>
      </View>

      <View style={styles.progressContainer} dataSet={{ media: ids.progressContainer }}>
        <Pressable
              ...
              return (
                <TouchableWithoutFeedback key={index} onPress={() => handlePress()}>
                  <View style={[
                    ...
                    index < lessonIdx ? { backgroundColor: color.primary } : null,
                  ]}>
                    <View style={[
                      styles.contentProgress,
                      index === lessonIdx ? { width: `${progressPercentage}%` } : null,
                    ]} />
                  </View>
        ...
        <Pressable onHoverIn={() => setIsHoveredRight(true)}
          onHoverOut={() => setIsHoveredRight(false)} dataSet={{ media: ids.noneDisplay }}>
          <MaterialCommunityIcons name="chevron-right" size={30} color={color.onSurface}
